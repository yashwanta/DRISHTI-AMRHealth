...
function plantFromServer(serverName: string): string {
  ...
  if (low.includes("hop") || low.includes("hopkinsville"))
    return "Hopkinsville";
  if (low.includes("shelby") || low.includes("shelbyville"))
    return "Shelbyville";
  return serverName || "—";
}
...
function ipFromMessage(msg: string, source?: string): string {
  ...
  if (m) return m[1];
  const ips = Array.from(msg.matchAll(anyIPRe), (x) => x[1]).filter(
    (ip) => ip !== "127.0.0.1" && ip !== "0.0.0.0",
  );
  return ips[0] ?? "";
}
...
function LogRow({
  ...
  event: LogEvent;
  ipByRobot: Record<string, string>;
}) {
  ...
  const ip =
    ipFromMessage(event.message, event.source) ||
    (robot !== "—" ? (ipByRobot[robot] ?? "") : "");
  ...
  return (
    <>
      <tr className="border-b border-gray-800 hover:bg-gray-800/60 transition-colors group">
        ...
        <td className="px-3 py-3 whitespace-nowrap align-top">
          {robot !== "—" ? (
            <button
              onClick={() =>
                nav(
                  `/agent?robot=${encodeURIComponent(robot)}&plant=${encodeURIComponent(plant !== "—" ? plant : "")}`,
                )
              }
        ...
        <td className="px-3 py-3 whitespace-nowrap align-top">
          <span
            className={`text-[10px] font-semibold px-2 py-0.5 rounded-full uppercase ${SEV_CLS[event.severity] ?? SEV_CLS.info}`}
          >
            {event.severity}
          </span>
        </td>
        ...
        <td className="px-3 py-3 align-top max-w-md">
          ...
          {hasRaw && (
            ...
            >
              {expanded ? (
                <>
                  <ChevronUp size={11} /> Hide raw
                </>
              ) : (
                <>
                  <ChevronDown size={11} /> Show raw
                </>
              )}
            </button>
          )}
...
const sfTcpRe =
  /^(\S+)\s+\d+\s+\d+\s+[\d.]+:\d+\s+(10\.222\.(?!10\.)\d+\.\d+):(\d+)/;

function SfTcpPanel({ tcpEvents }: { tcpEvents: LogEvent[] }) {
  const [diagnosing, setDiagnosing] = useState("");
  const [diagnostics, setDiagnostics] = useState<
    Record<string, AMRTCPDiagnostic | { error: string }>
  >({});
  const amrMap: Record<
    string,
    { states: Record<string, number>; ports: Set<number> }
  > = {};
  ...
  const diagnose = async (ip: string, ports: Set<number>) => {
    setDiagnosing(ip);
    try {
      const result = await diagnoseAMRTCP(
        ip,
        [...ports].sort((a, b) => a - b),
      );
      setDiagnostics((current) => ({ ...current, [ip]: result }));
    } catch (error: any) {
    ...
    <div className="mx-6 mt-4 mb-2 rounded-lg border border-gray-700 bg-gray-800/40 overflow-hidden">
      <div className="px-4 py-2 border-b border-gray-700 flex items-center gap-2">
        ...
        <span className="text-xs font-semibold text-gray-300">
          Springfield — Live AMR TCP Connections
        </span>
        <span className="ml-auto text-[10px] text-gray-500">
          {entries.length} AMRs · live_amr_tcp snapshot · IP is the only
          identifier available (no Roboshop logs on this server)
        </span>
      </div>
      <table className="w-full">
        ...
        <tbody>
          {entries.map(([ip, data]) => {
            const hasProblems =
              "FIN-WAIT-1" in data.states ||
              "SYN-SENT" in data.states ||
              "CLOSE-WAIT" in data.states;
            const total = Object.values(data.states).reduce((a, b) => a + b, 0);
            const stateStr = Object.entries(data.states)
              .sort(([a], [b]) => a.localeCompare(b))
              .map(([s, n]) => `${s}×${n}`)
              .join("  ");
            const diagnostic = diagnostics[ip];
            return (
              <Fragment key={ip}>
                <tr className="border-t border-gray-700/40 hover:bg-gray-700/30 transition-colors">
                  <td className="px-4 py-2.5 font-mono text-sm text-gray-200">
                    {ip}
                  </td>
                  <td className="px-4 py-2.5 text-sm text-gray-400">{total}</td>
                  <td
                    className={`px-4 py-2.5 text-sm font-medium ${hasProblems ? "text-amber-400" : "text-green-500"}`}
                  >
                    {hasProblems ? "⚠ " : "✓ "}
                    {stateStr}
                  </td>
                  <td className="px-4 py-2 text-right">
                    <button
                      type="button"
                      onClick={() => void diagnose(ip, data.ports)}
                      disabled={diagnosing === ip}
                      className="rounded border border-blue-500/60 bg-blue-950/40 px-2.5 py-1 text-xs text-blue-300 hover:bg-blue-900/60 disabled:opacity-50"
                    >
                      {diagnosing === ip ? "Checking..." : "Diagnose"}
                    </button>
                  </td>
                </tr>
                {diagnostic && (
                      ...
                      {"error" in diagnostic ? (
                        ...
                        <div className="space-y-2">
                          <div className="flex flex-wrap gap-3 text-xs text-gray-400">
                            <span>Checked from {diagnostic.server}</span>
                            <span>
                              {new Date(diagnostic.checked_at).toLocaleString()}
                            </span>
                            <span>Ports {diagnostic.ports.join(", ")}</span>
                          </div>
                          <pre className="max-h-64 overflow-auto whitespace-pre-wrap rounded bg-black/70 p-3 font-mono text-[11px] text-green-300">
                            {diagnostic.output ||
                              diagnostic.error ||
      ...
      <div className="flex items-center justify-between px-6 py-4 border-b border-gray-700 gap-4 flex-wrap">
        <div>
          ...
          <p className="text-xs text-gray-400 mt-0.5">
            AMR-specific events · {filtered.length} shown
            {robotSet.size > 0 ? ` · ${robotSet.size} robots` : ""}
          </p>
        </div>
      ...
      <div className="flex-1 overflow-y-auto">
        {isLoading && (
          <div className="flex items-center justify-center py-20 text-gray-500">
            <RefreshCw size={18} className="animate-spin mr-2" /> Loading AMR
            logs…
          </div>
        )}
        {error && !isLoading && (
        ...
        {!isLoading && !error && filtered.length > 0 && (
          <table className="w-full text-sm">
            <thead className="bg-gray-800/80 border-b border-gray-700 sticky top-0 z-10">
              <tr>
                <th className="px-4 py-2.5 text-left text-[11px] font-semibold text-gray-400 uppercase tracking-wide whitespace-nowrap">
                  Timestamp
                </th>
        ...
        {!isLoading && !error && filtered.length > 0 && (
          <div className="px-4 py-3 text-[11px] text-gray-600 border-t border-gray-800">
            Showing {filtered.length} events · Click a robot name to open Agent
            investigation · "Show raw" expands the original log line
          </div>
        )}
      </div>
    </div>
