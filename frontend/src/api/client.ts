...
export const getMe = () =>
  api
    .get<{
      username: string;
      role: string;
      permissions: import("../types").AdminPermission[];
    }>("/auth/me")
    .then((r) => r.data);
export const logout = () =>
...
export const syncServer = (id: number) =>
  api.post<{ job_id: number }>(`/servers/${id}/sync`).then((r) => r.data);
...
export const syncAll = (assetType?: "server" | "endpoint") =>
  api
    .post<SyncAllResponse>("/sync/all", null, {
...
export const explainLogErrors = (events: LogEvent[], context: string) =>
  api
    .post<AgentLogExplanation>("/logs/agent-explain", {
      context,
...
export const getTimeline = () =>
  api.get<TimelinePoint[]>("/timeline").then((r) => r.data);
export const getSyncHistory = () =>
  api.get<SyncJob[]>("/sync-history").then((r) => r.data);

export const getServerStats = () =>
  api.get("/server-stats").then((r) => r.data);

export const deepSync = (id: number, since: string) =>
  api
    .post(`/servers/${id}/deep-sync?since=${encodeURIComponent(since)}`)
    .then((r) => r.data);
...
export const getIncidentSummary = (params: IncidentSummaryParams) =>
  api
    .get<IncidentSummary>("/incidents/summary", { params })
    .then((r) => r.data);
...
export const createUser = (data: AppUserRequest) =>
  api.post<AppUser>("/users", data).then((r) => r.data);
export const updateUser = (id: number, data: AppUserRequest) =>
  api.put<AppUser>(`/users/${id}`, data).then((r) => r.data);
export const deleteUser = (id: number) => api.delete(`/users/${id}`);

// RDS / RoboWatch
export const getRdsPlants = () =>
  api.get<PlantConfig[]>("/rds/plants").then((r) => r.data);
export const getRdsConnectionStatus = (plant: string) =>
  api
    .get<RdsConnectionStatus>(`/rds/status/${encodeURIComponent(plant)}`)
    .then((r) => r.data);
export const testRdsConnection = (plant: string) =>
  api
    .post<RdsTestResult>(`/rds/test/${encodeURIComponent(plant)}`)
    .then((r) => r.data);
export const saveRdsCredentials = (
  plant: string,
  username: string,
  password: string,
) =>
  api
    .put<{
      status: string;
    }>(`/rds/credentials/${encodeURIComponent(plant)}`, { username, password })
    .then((r) => r.data);
export const discoverRdsSources = (plant: string) =>
  api
    .post<RdsSourceDiscovery>(`/rds/discover/${encodeURIComponent(plant)}`)
    .then((r) => r.data);
export const fetchRdsLogs = (plant: string) =>
  api
    .post<{
      event_count?: number;
      message?: string;
    }>(`/rds/fetch/${encodeURIComponent(plant)}`)
    .then((r) => r.data);
...
export const getAgentJob = (jobId: string) =>
  api
    .get<AgentJob>(`/agent/jobs/${encodeURIComponent(jobId)}`)
    .then((r) => r.data);
export const getAgentRobots = (plant: string) =>
  api
    .get<{ id: string; name: string }[]>("/agent/robots", { params: { plant } })
    .then((r) => r.data);
...
export const getAMRTimeline = (
  plant?: string,
  robot?: string,
  range?: TimeRange,
) =>
  api
    .get<AMRDropEvent[]>("/amr/timeline", {
      params: {
        ...(plant ? { plant } : {}),
        ...(robot ? { robot } : {}),
...
export const getAMRRobotSummary = (
  robot: string,
  plant?: string,
  range?: TimeRange,
) =>
  api
    .get<AMRRobotSummary>("/amr/robot", {
      params: {
        robot,
        ...(plant ? { plant } : {}),
        ...(range?.from ? { from: range.from } : {}),
...
export const getAMRBadZones = (plant?: string, range?: TimeRange) =>
  api
    .get<
      AMRBadZone[]
    >("/amr/badzones", { params: { ...(plant ? { plant } : {}), ...(range?.from ? { from: range.from } : {}), ...(range?.to ? { to: range.to } : {}) } })
    .then((r) => r.data);
...
export const getAMRSummary = (
  plant?: string,
  robot?: string,
  range?: TimeRange,
) =>
  api
    .get<AMRSummary>("/amr/summarize", {
      params: {
        ...(plant ? { plant } : {}),
        ...(robot ? { robot } : {}),
        ...(range?.from ? { from: range.from } : {}),
...
export interface AMRTCPDiagnostic {
  ok: boolean;
  ip: string;
  server: string;
  ports: number[];
...
export const diagnoseAMRTCP = (ip: string, ports: number[]) =>
  api
    .post<AMRTCPDiagnostic>("/amr/tcp-diagnostics", { ip, ports })
    .then((r) => r.data);
...
export interface WifiSurveyRoutePointInput {
  session_id: number;
  plant_id: string;
  map_id: string;
  map_version: string;
...
export const saveWifiHeatmapPoint = (
  point: WifiHeatmapPointInput,
  toleranceSeconds = 15,
) =>
  api
    .post("/wifi-heatmap/points", point, {
      params: { tolerance_seconds: toleranceSeconds },
    })
    .then((r) => r.data);
export const saveWifiSurveyRoutePoint = (point: WifiSurveyRoutePointInput) =>
  api.post("/wifi-heatmap/route-points", point).then((r) => r.data);
export const startWifiHeatmapSession = (data: Record<string, unknown>) =>
  api
    .post<{
      id: number;
      status: string;
      started_at: string;
    }>("/wifi-heatmap/sessions", data)
    .then((r) => r.data);
export const stopWifiHeatmapSession = (id: number) =>
  api.post(`/wifi-heatmap/sessions/${id}/stop`).then((r) => r.data);
export const listWifiHeatmapSessions = () =>
  api.get("/wifi-heatmap/sessions").then((r) => r.data);
export const queryWifiHeatmap = (params: Record<string, string | number>) =>
  api.get("/wifi-heatmap/query", { params }).then((r) => r.data);
