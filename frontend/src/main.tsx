...
import plantMap from "./assets/plant-map.svg";
import "./styles.css";
...
type WifiPoint = { plant: string; amr: string; x: number; y: number; rssi: number; quality: "Good" | "Weak" | "Poor" | "Critical"; ap: string; ssid: string; channel: string; band: string; reconnect: boolean; disconnect: boolean; offline: boolean; roaming: boolean; time: string; imported?: boolean; source?: string };
...
type AppState = {
  amrs: AMR[]; wifiPoints: WifiPoint[]; logs: LogEntry[]; badZones: BadZone[];
...
function normalizeRdsCoreResponse(payload: any, plant: string, connection?: APIConnection): NormalizedRds {
  ...
  const points: WifiPoint[] = reports.map((item: any) => {
    ...
    return {
      ...
      ap: "RDS Core position only", ssid: "unknown", channel: "unknown", band: "unknown", reconnect: false, disconnect: disconnected, offline: disconnected, roaming: false, time: core.create_on || importedAt, imported: true, source
    };
  });
  const logs: LogEntry[] = [];
...
function App() {
  ...
  async function pullLiveCore() {
    ...
    try {
      ...
      mergeImport(normalizeRdsCoreResponse(await response.json(), selectedImportPlant, connection));
    } catch (error) {
  ...
  return <div className="app-shell">
    ...
    <main className="main-content"><header className="topbar"><div><h1>{view === "dashboard" ? "AMR Health Dashboard" : view[0].toUpperCase() + view.slice(1)}</h1><p>Go backend, React UI, local config, and local-only RDS snapshots.</p></div><div className="topbar-controls"><label className="field compact"><span>Plant</span><select value={plantFilter} onChange={(e) => setPlantFilter(e.target.value)}><option>All</option>{plantOptions.map((plant) => <option key={plant}>{plant}</option>)}</select></label><label className="field search-field"><span>Search</span><input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="AMR, IP, zone, AP, topic" /></label></div></header>
      ...
      {view === "heatmap" && <section className="view active-view"><section className="panel heatmap-panel"><div className="panel-header"><div><h2>AMR Wi-Fi Heat Map</h2><p>RDS positions are shown as position-only points until Wi-Fi telemetry is connected.</p></div><label className="field compact"><span>Signal</span><select value={signalFilter} onChange={(e) => setSignalFilter(e.target.value)}><option>All</option><option>Good</option><option>Weak</option><option>Poor</option><option>Critical</option></select></label></div><div className="map-shell"><img src={state.uploadedMap || plantMap} alt="Plant floor layout" /><div className="heatmap-overlay">{filteredPoints.map((point, index) => <button key={index} className={`map-point ${point.quality.toLowerCase()}`} style={{ left: `${point.x}%`, top: `${point.y}%` }} title={`${point.amr} ${point.quality} ${point.rssi} dBm via ${point.ap}`} data-label={`${point.amr} ${point.rssi} dBm ${point.ap}`}></button>)}</div></div></section></section>}
