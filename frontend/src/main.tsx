...
type WifiTestResult = { ok: boolean; method: string; host: string; message: string; output?: string; rssi?: number; quality?: string };
type WifiDiscoverResult = { ok: boolean; plant: string; amr: string; host: string; command?: string; message: string; output?: string; rssi?: number; quality?: WifiPoint["quality"] | "Unknown" };
    ...
    try {
      ...
      setState((current) => ({
        ...
        wifiPoints: current.wifiPoints.map((point) => {
          ...
          return match && point.plant === match.plant ? { ...point, rssi: match.rssi!, quality: match.quality as WifiPoint["quality"], ap: `Robot ${match.host}`, ssid: "AMR SSH RSSI", source: "AMR SSH Auto-Discovery", time: new Date().toISOString() } : point;
        }),
        amrs: current.amrs.map((amr) => {
          const match = realResults.get(amr.name);
          return match && amr.plant === match.plant ? { ...amr, rssi: match.rssi!, ap: `Robot ${match.host}`, ssid: "AMR SSH RSSI" } : amr;
        }),
